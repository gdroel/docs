---
title: 'Custom Migration UI'
description: 'Build your own migration experience with ZSMigrationManager'
icon: 'wand-magic-sparkles'
---

`ZSMigrationManager` is the state machine behind `ZSMigrateTipView`. If the built-in view doesn't fit your design, use the manager directly to build a fully custom migration experience while reusing all the eligibility logic, checkout orchestration, and state tracking.
...
The migration flow is a linear state machine. Each state represents a step in the user journey.
...
| `ineligible` | User doesn't qualify (no StoreKit subscription, already migrated, no campaign configured). | `nil` |
...
| `completed` | User opened Apple subscription management. Migration flow is done. | Available |
...
|----------|------|-------------|
| `state` | `MigrationOffer.State` | Current state of the migration flow. |
...
| Field | Type | Description |
|-------|------|-------------|
| `productId` | `String` | The web product ID to offer for migration. |
...
All of these are configured in your [dashboard migration campaign](/iap/dashboard#migration-campaigns).
...
Set `ZSMigrationManager.demoMode = true` before bootstrapping to preview the migration UI without an active StoreKit subscription. The manager synthesizes a demo offer so you can test your custom UI in the simulator.

```swift
ZSMigrationManager.demoMode = true
ZeroSettle.configure(publishableKey: "pk_test_...")
await ZeroSettle.bootstrap(userId: "demo_user")
```

## Full Example

A complete custom migration card with loading, error, checkout, and completion states:

```swift
struct CustomMigrationCard: View {
