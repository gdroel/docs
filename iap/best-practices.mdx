...
Initialize the SDK as early as possible in your app's lifecycle. On iOS, `configure()` is async — call it from a `.task {}` modifier. When you provide a `userId`, it automatically fetches products, warms up the first product's `PaymentIntent`, and restores entitlements. On Android, `configure()` is synchronous and should be called in `Application.onCreate()`, then call `bootstrap()` to fetch products and restore entitlements:
...
On iOS, `ZSPaymentSheet` provides the best conversion rates because the user stays in your app. On Android, use `ZeroSettle.purchase()` which opens a Custom Tab for the web checkout:
...
|----------|----------|-------------------|
| Complete purchase (payment sheet) | iOS | Transaction returned, entitlements update |
| Complete purchase (Safari) | iOS | Universal link callback, entitlements update |
| Complete purchase (Custom Tab) | Android | Transaction returned, entitlements update |
| Cancel payment sheet | iOS | `.cancelled` error, no entitlement change |
| Cancel Safari checkout | iOS | No entitlement change, no error shown |
