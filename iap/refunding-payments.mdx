...
The refund will be processed immediately, and the customer's entitlements will be updated to reflect the change. The `isActive` property on the affected `Entitlement` will be set to `false`, and the corresponding `ZSTransaction` status will change to `.refunded`.
