...
```tsx React Native
await ZeroSettle.configure({ publishableKey: 'pk_live_...' });
...
const entitlements = await ZeroSettle.restoreEntitlements(auth.currentUser.id);
```

```dart Flutter
await ZeroSettle.instance.configure(publishableKey: 'pk_live_...');
...
// Configure
ZeroSettle.configure(context, ZeroSettle.Configuration(publishableKey = "pk_live_..."))
...
const transaction = await ZeroSettle.purchase('premium_lifetime', '');
```
