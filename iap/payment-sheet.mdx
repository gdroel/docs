...
```kotlin Kotlin
// Non-dismissable sheets are not available on Android.
// The payment sheet UI is handled by ZSPaymentSheetActivity.
// See the Android section below for the purchase API.
val transaction = ZeroSettle.purchase(
    activity = this,
...
```tsx React Native
// Non-dismissable sheets are not available in React Native.
// Use ZeroSettle.purchase() for the standard checkout flow.
const transaction = await ZeroSettle.purchase(product.id, currentUser.id);
```

```dart Flutter
// Non-dismissable sheets are iOS-only (SwiftUI).
// Use presentPaymentSheet() for the standard checkout flow.
final transaction = await ZeroSettle.instance.presentPaymentSheet(
  productId: product.id,
  userId: currentUser.id,
...
<Info>
  The SwiftUI-specific APIs above (`isPresented:`, `item:`, custom headers, `dismissable:`) are iOS-only. On Android, use `ZeroSettle.purchase()` which handles all presentation automatically. See the [Android section](#android) below.
</Info>
...
```kotlin Kotlin
// UIKit is iOS-only. On Android, use ZeroSettle.purchase().
// Preloading and dismissable options are not available on Android.
val transaction = ZeroSettle.purchase(
    activity = this,
    productId = product.id,
    userId = currentUser.id,
