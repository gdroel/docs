...
The checkout mode is controlled by your remote config. When set to `WEBVIEW` (the default), `purchase()` launches the embedded payment sheet. For `CUSTOM_TAB` or `EXTERNAL_BROWSER` modes, it opens a Chrome Custom Tab or the default browser instead.

<Note>
  The iOS-specific APIs (`zsPaymentSheet` modifier, `warmUp()`, `preload()`) are not available on Android. On Android, use `ZeroSettle.purchase()` directly.
</Note>
