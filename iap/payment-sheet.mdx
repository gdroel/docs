...
```kotlin Kotlin
@Composable
fun PaywallScreen(product: Product) {
    ...
    Button(onClick = {
        scope.launch {
            try {
                ...
                unlockContent(transaction.productId)
            } catch (e: ZeroSettleError.Cancelled) {
...
The `checkoutSheet` modifier uses an item-based binding by default. Pass an optional `Product?` binding — the sheet presents when the binding is non-nil and automatically sets it back to `nil` on dismiss:
...
The `checkoutSheet` modifier uses an `item:` binding (`Binding<Product?>`) that mirrors SwiftUI's `.sheet(item:)` pattern. Set the binding to a product to present the sheet, and it automatically resets to `nil` on dismiss.
...
fun PaywallScreen(product: Product) {
    ...
    Button(onClick = {
        scope.launch {
            ...
            } catch (e: ZeroSettleError.NotConfigured) {
...
`ZeroSettle.purchase()` is a suspend function that throws on failure. Errors are thrown as subtypes of `ZeroSettleError`:
...
try {
    ...
    // SDK not configured — call ZeroSettle.configure() first
} catch (e: ZeroSettleError.UserIdRequired) {
...
The completion handler receives a `Result<CheckoutTransaction, Error>`. Payment-specific errors are of type `PaymentSheetError` (iOS) or `ZeroSettleError` (Android):
...
#### Android (`ZeroSettleError`)

| Error | Description |
|-------|-------------|
| `ZeroSettleError.Cancelled` | User dismissed the payment sheet |
| `ZeroSettleError.NotConfigured` | SDK hasn't been configured yet — call `configure()` first |
| `ZeroSettleError.CheckoutFailed(reason)` | Checkout failed — inspect `reason` for details (see `CheckoutFailure` below) |
| `ZeroSettleError.UserIdRequired(productId)` | A `userId` is required for subscriptions and non-consumable products |
| `ZeroSettleError.TransactionVerificationFailed(detail)` | Transaction completed but server-side verification failed |
...
<Note>
  `.cancelled` / `ZeroSettleError.Cancelled` is not a real error — it just means the user changed their mind. Don't show an error message for cancellations.
</Note>
...
Free trials are configured **server-side per product** in the [ZeroSettle dashboard](https://dashboard.zerosettle.io). The SDK reads the trial configuration from your product catalog — there is no client-side trial parameter to pass.

The catalog response exposes two fields you can read on each `Product`:

| Field | Type | Description |
|---|---|---|
| `isTrialEligible` | `Bool` | Whether the current user is eligible for a trial on this product (server determines based on prior trial usage). |
| `freeTrialDuration` | `String?` | Human-readable trial length (e.g., `"7 days"`) when a trial is configured. |
...
if product.isTrialEligible {
    ...
    print("Trial: \(product.freeTrialDuration ?? "")")
}

// Just present the sheet — no trial parameter needed
.checkoutSheet(item: $selectedProduct, userId: currentUser.id) { result in
    handleResult(result)
}
```
...
```kotlin Kotlin
// Free trial configuration is server-side. The payment sheet displays the trial automatically.
if (product.isTrialEligible) {
    // Update your CTA copy
}
```
...
|---|---|---|---|
| `stripeCustomerId` | `String?` (Optional) | `nil` | Existing Stripe Customer ID (`cus_xxx`). When provided, the checkout PaymentIntent is attached to this customer instead of creating a new one. Useful for showing a consolidated [Stripe Billing Portal](https://docs.stripe.com/customer-management/integrate-customer-portal) view across all of the user's purchases. |
