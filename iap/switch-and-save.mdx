...
```tsx React Native
import { OfferTipView } from 'react-native-zerosettle';
...
<Note>
  `MigrationTipView` targets users switching from **Apple StoreKit** subscriptions to direct billing. On Android, the equivalent switch and save flow from Google Play Billing is not yet supported by this component.
</Note>
...
```tsx React Native
import { resetMigrateTipState } from 'react-native-zerosettle';
...
```tsx React Native
import { forceUSARegion, resetRegionOverride } from 'react-native-zerosettle';
