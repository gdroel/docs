...
```kotlin Kotlin
// ZSMigrateTipView is currently iOS-only (StoreKit → web checkout migration).
// Android migration from Play Billing is not yet supported by this component.
```
...
|-----------|------|-------------|
| `backgroundColor` | `Color` (SwiftUI, Flutter) / `string` (React Native) | Background color for the card. Pass a `Color` in SwiftUI and Flutter, or a hex string like `"#1E1E1E"` in React Native. |
...
| `onDismiss` | `(() -> Void)?` | `nil` | Closure invoked when the view is dismissed (close button, auto-dismiss after completion, or state becomes ineligible). |
...
```tsx React Native
// Not available in React Native — use the native iOS debug tools.
```

```dart Flutter
// Not available in Flutter — use the native iOS debug tools.
```
...
<CardGroup cols={2}>
  <Card title="Custom Migration UI" icon="wand-magic-sparkles" href="/iap/migration-manager">
    Build your own migration experience with ZSMigrationManager
  </Card>
