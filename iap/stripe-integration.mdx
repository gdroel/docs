...
<Note>
  If you manually configure Stripe webhooks for ZeroSettle (BYOS only), include `price.deleted` and `price.updated` events. The backend invalidates cached price metadata when Stripe disables or removes a price, preventing stale `stripe_price_id` reuse during checkout.
</Note>
