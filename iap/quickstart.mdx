...
```swift
let catalog = try await ZeroSettleIAP.shared.fetchProducts(userId: currentUser.id)
...
`fetchProducts()` returns a ``ProductCatalog`` containing both the product list and remote configuration. Products are also cached in `ZeroSettleIAP.shared.products` for convenience.
...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .task {
            let catalog = try? await iap.fetchProducts(userId: Auth.currentUser.id)

            // Warm up the PaymentIntent cache for faster first open
            if let first = catalog?.products.first {
                await ZSPaymentSheet.warmUp(productId: first.id, userId: Auth.currentUser.id)
            }
