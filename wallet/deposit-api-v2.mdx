...
Prevent duplicate deposits by providing an idempotency key:
...
do {
    ...
    // If user retries with same key, same transaction is returned
    // without creating a duplicate deposit
    
} catch {
    // Handle error - safe to retry with same idempotency key
}
```

<Tip>
  Generate idempotency keys using `UUID().uuidString` or your own deterministic key generation. Keys are valid for 24 hours.
</Tip>
...
<Note>
  **Prefer the prebuilt UI?** Check out [`ZSDepositView`](/wallet/deposit-view) for a ready-made deposit screen that uses this API internally.
</Note>
